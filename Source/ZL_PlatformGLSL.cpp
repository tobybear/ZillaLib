...
namespace ZLGLSL
{
	...
	GLuint ATTR_COLOR = 0;
	GLuint ATTR_TEXCOORD = 0;

	GLuint _COLOR_PROGRAM = 0;
	GLuint COLOR_UNI_MVP;
	GLuint COLOR_ATTR_POSITION;
	GLuint COLOR_ATTR_COLOR;

	GLuint _TEXTURE_PROGRAM = 0;
	GLuint TEXTURE_UNI_MVP;
	GLuint TEXTURE_ATTR_POSITION;
	GLuint TEXTURE_ATTR_COLOR;
	GLuint TEXTURE_ATTR_TEXCOORD;
	...
	GLuint CreateShaderOfType(GLenum type, const char *shader_src)
	{
		...
		if (compiled == 0)
		{
			...
			if (info_len > 1)
			{
				...
				ZL_LOG1("ZLGLSL", "Shader Source: %s", shader_src);
				free(info_log);
			}
			#endif
	...
	GLuint CreateProgramFromVertexAndFragmentShaders(const char *vertex_shader_src, const char *fragment_shader_src, const char* position_attr_name)
	{
		...
		if (!(vertex_shader = CreateShaderOfType(GL_VERTEX_SHADER, vertex_shader_src))) return 0;
		if (!(fragment_shader = CreateShaderOfType(GL_FRAGMENT_SHADER, fragment_shader_src))) { glDeleteShader(vertex_shader); return 0; }

		// Create the program object and attach the shaders.
		if (!(program_object = glCreateProgram())) return 0;
		glAttachShader(program_object, vertex_shader);
		glAttachShader(program_object, fragment_shader);

		//attribute bound to index 0 must always be an enabled array attribute - so force position to it
		glBindAttribLocation(program_object, 0, position_attr_name);
		...
		//ZL_LOG3("GLSL", "Loaded Shader Program #%d with Vertex Shader:\n%s\n     And Fragment Shader:\n%s\n\n", program_object, vertex_shader_src, fragment_shader_src);
	...
	bool CreateShaders()
	{
		const char color_vertex_shader_src[] =
			//"#ifdef GL_ES\nprecision lowp float;\n#endif\n"
			#ifdef ZL_VIDEO_OPENGL_ES2
			"precision lowp float;"
			#endif
		...
		const char color_fragment_shader_src[] =
			//"#ifdef GL_ES\nprecision lowp float;\n#endif\n"
			#ifdef ZL_VIDEO_OPENGL_ES2
			"precision lowp float;"
			#endif
		...
		if (!(_COLOR_PROGRAM = CreateProgramFromVertexAndFragmentShaders(color_vertex_shader_src, color_fragment_shader_src, "a_position"))) return false;
		COLOR_UNI_MVP       = (GLuint)glGetUniformLocation(_COLOR_PROGRAM, "u_mvpMatrix");
		COLOR_ATTR_POSITION = (GLuint)glGetAttribLocation(_COLOR_PROGRAM, "a_position");
		COLOR_ATTR_COLOR    = (GLuint)glGetAttribLocation(_COLOR_PROGRAM, "a_color");

		// Load the shaders and get a linked program object
		if (!(_TEXTURE_PROGRAM = CreateProgramFromVertexAndFragmentShaders(texture_vertex_shader_src, texture_fragment_shader_src, "a_position"))) return false;
		TEXTURE_UNI_MVP       = (GLuint)glGetUniformLocation(_TEXTURE_PROGRAM, "u_mvpMatrix");
		TEXTURE_ATTR_POSITION = (GLuint)glGetAttribLocation(_TEXTURE_PROGRAM, "a_position");
		TEXTURE_ATTR_COLOR    = (GLuint)glGetAttribLocation(_TEXTURE_PROGRAM, "a_color");
		TEXTURE_ATTR_TEXCOORD = (GLuint)glGetAttribLocation(_TEXTURE_PROGRAM, "a_texcoord");

		glEnableVertexAttribArray(0); //0 = POSITION = always required
	...
	{
		ActiveProgram = COLOR;
		if (ATTR_TEXCOORD) glDisableVertexAttribArray(ATTR_TEXCOORD);
		glUseProgram(_COLOR_PROGRAM);
		UNI_MVP = COLOR_UNI_MVP;
		ATTR_POSITION = COLOR_ATTR_POSITION;
		ATTR_COLOR = COLOR_ATTR_COLOR;
		ATTR_TEXCOORD = 0;
		MatrixApply();
	}
	...
	void _TEXTURE_PROGRAM_ACTIVATE()
	{
		...
		ATTR_POSITION = TEXTURE_ATTR_POSITION;
		ATTR_COLOR = TEXTURE_ATTR_COLOR;
		ATTR_TEXCOORD = TEXTURE_ATTR_TEXCOORD;
		glEnableVertexAttribArray(ATTR_TEXCOORD); //always required
		MatrixApply();
	}
}
...
#include "ZL_String.h"
struct ZL_Shader_Impl;
struct ZL_PostProcess_Impl;
static std::vector<ZL_Shader_Impl*> *pLoadedShaders = NULL;
static std::vector<ZL_PostProcess_Impl*> *pLoadedPostProcesses = NULL;
#endif

struct ZL_Shader_Impl : ZL_Impl
{
	GLuint PROGRAM;
	GLint UNI_MVP, ATTR_POSITION, ATTR_COLOR, ATTR_TEXCOORD, UNI_1, UNI_2;
	#ifdef ZL_VIDEO_WEAKCONTEXT
	...
	{
		if (!(PROGRAM = ZLGLSL::CreateProgramFromVertexAndFragmentShaders((code_vertex ? code_vertex : texture_vertex_shader_src), (code_fragment ? code_fragment : texture_fragment_shader_src), "a_position"))) return;
		UNI_MVP       = (GLuint)glGetUniformLocation(PROGRAM, "u_mvpMatrix");
		ATTR_POSITION = (GLuint)glGetAttribLocation(PROGRAM, "a_position");
		ATTR_COLOR    = (GLuint)glGetAttribLocation(PROGRAM, "a_color");
		ATTR_TEXCOORD = (GLuint)glGetAttribLocation(PROGRAM, "a_texcoord");
	...
	~ZL_Shader_Impl()
	{
		...
		#ifdef ZL_VIDEO_WEAKCONTEXT
		for (std::vector<ZL_Shader_Impl*>::iterator it = pLoadedShaders->begin(); it != pLoadedShaders->end(); ++it)
			if (*it == this) { pLoadedShaders->erase(it); break; }
		#endif
	}

	void Activate(scalar *uni1, scalar *uni2)
	{
		ZLGLSL::ActiveProgram = ZLGLSL::CUSTOM;
		glUseProgram(PROGRAM);
		ZLGLSL::UNI_MVP = UNI_MVP;
		ZLGLSL::ATTR_POSITION = ATTR_POSITION;
		ZLGLSL::ATTR_COLOR = ATTR_COLOR;
		ZLGLSL::ATTR_TEXCOORD = ATTR_TEXCOORD;
		glEnableVertexAttribArray(ZLGLSL::ATTR_POSITION);
		glEnableVertexAttribArray(ZLGLSL::ATTR_TEXCOORD);
		ZLGLSL::MatrixApply();
...
void ZL_Shader::Activate()
{
	...
	ZLGLSL::ATTR_POSITION = impl->ATTR_POSITION;
	ZLGLSL::ATTR_COLOR = impl->ATTR_COLOR;
	ZLGLSL::ATTR_TEXCOORD = impl->ATTR_TEXCOORD;
	glEnableVertexAttribArray(ZLGLSL::ATTR_POSITION);
	glEnableVertexAttribArray(ZLGLSL::ATTR_TEXCOORD);
	ZLGLSL::MatrixApply();
}
...
static const char postprocess_vertex_shader_src[] =
	//"#ifdef GL_ES\nprecision lowp float;\n#endif\n"
	#ifdef ZL_VIDEO_OPENGL_ES2
	"precision lowp float;"
	#endif
	"attribute vec4 a_position;attribute vec2 a_texcoord;varying " /*"lowp "*/ "vec2 v_texcoord;"
	"void main(){v_texcoord=a_texcoord;gl_Position=a_position;}";

struct ZL_PostProcess_Impl : ZL_Impl
{
	GLuint PROGRAM;
	GLint ATTR_POSITION, ATTR_TEXCOORD, UNI_1, UNI_2;
	ZL_Texture_Impl *tex;
	#ifdef ZL_VIDEO_WEAKCONTEXT
	...
	{
		if (!(PROGRAM = ZLGLSL::CreateProgramFromVertexAndFragmentShaders(postprocess_vertex_shader_src, code_fragment, "a_position"))) return;
		ATTR_POSITION = (GLuint)glGetAttribLocation(PROGRAM, "a_position");
		ATTR_TEXCOORD = (GLuint)glGetAttribLocation(PROGRAM, "a_texcoord");
		if (name_uniform_float_1) UNI_1 = (GLuint)glGetUniformLocation(PROGRAM, name_uniform_float_1);
		if (name_uniform_float_2) UNI_2 = (GLuint)glGetUniformLocation(PROGRAM, name_uniform_float_2);
	...
	void Apply(scalar *uni1, scalar *uni2)
	{
		...
		glVertexAttribPointer(ATTR_POSITION, 2, GL_SCALAR, GL_FALSE, 0, vec_fullbox);
		glVertexAttribPointer(ATTR_TEXCOORD, 2, GL_SCALAR, GL_FALSE, 0, tex_fullbox);
		if (uni1) glUniform1(UNI_1, (GLscalar)*uni1);
		if (uni2) glUniform1(UNI_2, (GLscalar)*uni2);
		//glDisable(GL_BLEND);
...
void RecreateAllProgramsOnContextLost()
{
	...
	if (pLoadedShaders)
		for (std::vector<ZL_Shader_Impl*>::iterator its = pLoadedShaders->begin(); its != pLoadedShaders->end(); ++its)
			(*its)->PROGRAM = ZLGLSL::CreateProgramFromVertexAndFragmentShaders(((*its)->strCodeVertex.length() ? (*its)->strCodeVertex.c_str() : texture_vertex_shader_src), ((*its)->strCodeFragment.length() ? (*its)->strCodeFragment.c_str() : texture_fragment_shader_src), "a_position");
	if (pLoadedPostProcesses)
		for (std::vector<ZL_PostProcess_Impl*>::iterator itp = pLoadedPostProcesses->begin(); itp != pLoadedPostProcesses->end(); ++itp)
			{ (*itp)->PROGRAM = ZLGLSL::CreateProgramFromVertexAndFragmentShaders(postprocess_vertex_shader_src, (*itp)->strCodeFragment.c_str(), "a_position"); }
