...
wasm-help:helpheader
	...
	$(info $( )        LLVM_ROOT = $(if $(ISWIN),D:)/path/to/wasm/llvm             #path to root of LLVM/Clang (with clang$(if $(ISWIN),.exe) etc. in it))
	$(info $( )        SYSTEM_ROOT = $(if $(ISWIN),D:)/path/to/wasm/system         #path to web platform system root (https://github.com/emscripten-core/emscripten/tree/incoming/system))
	$(info $( )        WASMOPT = $(if $(ISWIN),D:)/path/to/binaryen/wasm-opt$(if $(ISWIN),.exe,    )   #path to Wasm optimizer to create smaller .wasm files)
...
android-help:helpheader
	...
	$(info )
	$(info $( )    Optional you can specify the compile optimization level for release builds with RELEASE_OPTIMIZATION = #)
	$(info $( )    (s = optimize size, z = smaller size, lower performance, 2 = larger size, better performance))
	$(info )
