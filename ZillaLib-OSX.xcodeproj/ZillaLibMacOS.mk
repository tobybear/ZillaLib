...
ifeq ($(BUILD),RELEASE)
  ...
  ZLOUTDIR  := $(ZILLALIB_DIR)ZillaLib-OSX.xcodeproj/macos-build-debug
  APPOUTDIR := Debug-macos
...
ASSET_ZIP       := $(if $(ASSET_ALL_STARS),$(if $(ZLMACOS_ASSETS_OUTFILE),$(APPOUTDIR)/$(ZLMACOS_ASSETS_OUTFILE),$(APPOUTDIR)/$(ZillaApp).dat),)
endif

ifeq ($(if $(ASSET_ZIP),$(ZLMACOS_ASSETS_EMBED),),1)
...
clean:
	$(info Removing all build files ...)
	@$(CMD_DEL_FILES) $(ZLOUTDIR)/ZillaLib_$(CPUTYPE).a $(CPP_ZLOBJS) $(CPP_DEPOBJS) $(CC_DEPOBJS) $(M_DEPOBJS) $(CPP_ZLOBJS:%.o=%.d) $(CPP_DEPOBJS:%.o=%.d) $(CC_DEPOBJS:%.o=%.d) $(M_DEPOBJS:%.o=%.d)
...
DEPSOURCES += \
	...
	$(wildcard $(ZILLALIB_DIR)Source/sdl/video/cocoa/*.m)
endif
...
M_DEPOBJS   := $(addprefix $(ZLOUTDIR)/,$(patsubst   %.m,%$(OBJEXT),$(filter   %.m,$(DEPSOURCES))))
...
$(M_DEPOBJS)   : $(ZLOUTDIR)/%$(OBJEXT) : $(ZILLALIB_DIR)%.m   ; $(call   COMPILE,$@,$<,$(CC),$(GCCMFLAG) $(ZLFLAGS) $(DEPWARNINGS))
...
$(CPP_ZLOBJS)  : $(ZLOUTDIR)/%$(OBJEXT) : $(ZILLALIB_DIR)%.cpp ; $(call COMPILE,$@,$<,$(CXX),$(GCCMFLAG) $(CXXFLAGS) $(ZLFLAGS) -DGL_SILENCE_DEPRECATION -MMD -MP)
